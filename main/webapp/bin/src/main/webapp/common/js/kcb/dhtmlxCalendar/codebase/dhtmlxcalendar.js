	...
	this.setPosition = function(x, y) {
		...
		if (!this._hasParent) {
			if (typeof(x) != "undefined" && !isNaN(x)){
				if(document.body.offsetWidth-x<200)
				x = x-190;
				this.base.style.left = x+"px";
			} 
			if (typeof(y) != "undefined" && !isNaN(y)){
				if(document.body.offsetHeight-y<220)
					y = y-220;
	...
	this._show = function(inpId, autoHide) {
		...
		if (!inpId) {
			...
			if (this.pos == "right") {
				...
				//计算右边有没有溢出 孙东辉
				if(this._getLeft(this.i[inpId])+188>document.body.offsetWidth){
					this.base.style.left = (this._getLeft(this.i[inpId])-(this._getLeft(this.i[inpId])+188-document.body.offsetWidth)-10)+"px";
				}
			}
