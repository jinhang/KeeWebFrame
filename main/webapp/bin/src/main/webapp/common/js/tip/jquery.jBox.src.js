...
(function ($) {
    $.jBox = function (content, options) {
        ...
        boxHtml.push('<div class="jbox-help-title jbox-title-panel" style="height:29px;display:none;"></div>');
        boxHtml.push('<div class="jbox-help-button jbox-button-panel" style="height:29px;padding:5px 0 5px 0;display:none;"></div>');
        ...
        boxHtml.push('<a class="jbox-close" title="' + $.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer; top:6px; right:' + (6 + options.border) + 'px; width:31px; height:17px;' + (options.showClose ? '' : 'display:none;') + '"></a>');

        if (withTitle) {
            boxHtml.push('<div class="jbox-title-panel" style="height:29px;">');
            boxHtml.push('<div class="jbox-title' + (options.showIcon == true ? ' jbox-title-icon' : (options.showIcon == false ? '' : ' ' + options.showIcon)) + '" style="float:left;text-align:center; width:' + titleWidth + '; line-height:' + ($.browser.msie ? 29 : 29) + 'px; padding-left:' + (options.showIcon ? 18 : 5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (options.title == '' ? '&nbsp;' : options.title) + '</div>');
            boxHtml.push('</div>');
        }
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            states.push('<div class="jbox-button-panel" style="height:25px;padding:5px 20px 22px 0;text-align: right;' + (withButtons ? '' : 'display:none;') + '">');
            if (!options.isTip) {
            ...
            $.each(stateOptions.buttons, function (k, v) {
            	if(v)
            		states.push('<a class="butt2" value="' + v + '"><span>' + k + '</span></a>');
            	else
            		states.push('<a class="butt" value="' + v + '"><span>' + k + '</span></a>');
            });
            states.push('</div></div>');
        });
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            $state.children('.jbox-button-panel').children('a').click(function () {
        ...
        var removeBox = function () {
            ...
            if (options.isTip) {
                ...
                if (tip && tip.next == true) {
                    ...
                    $box.animate({ top: '-=0', opacity: 0 }, 500, removeBoxImpl);
                }
            }
            else {
        ...
        if (!options.isTip) {
        	...
        	if(options.error != true){
        		...
        		$box.find('.jbox-container,.jbox-content').addClass('jbox-tip-error-color');
        	}
    ...
    $.jBox.tipDefaults = { content: '', icon: 'none', top: '40%', width: 'auto', height: 'auto', opacity: 0, timeout: 3000, closed: function () { } };
    $.jBox.messagerDefaults = { content: '', title: 'jBox', icon: 'none', width: 350, height: 'auto', timeout: 3000, showType: 'slide', showSpeed: 100, border: 0, buttons: {}, buttonsFocus: 0, loaded: function () { }, submit: function (v, h, f) { return true; }, closed: function () { } };
    ...
    $.jBox.prompt = function (content, title, icon, options) {
        ...
        var html = [];
        html.push('html:');
        html.push('<div style="margin:10px;' + minHeight + 'padding-left:' + (options.icon == 'none' ? 0 : 20) + 'px;padding-top:10px;padding-bottom:10px;padding-right:20px;text-align:left;">');
    ...
    $.jBox.tip = function (content, icon, options) {
        ...
        var html = [];
        html.push('html:');
        html.push('<div style="min-height:18px;height:auto;margin:10px;');
        if(options.icon != 'none'){
    	   html.push('padding-left:30px;');
        }
        html.push('padding-top:0px;text-align:center;">');
