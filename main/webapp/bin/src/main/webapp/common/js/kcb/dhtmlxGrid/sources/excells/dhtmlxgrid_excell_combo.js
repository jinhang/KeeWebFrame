...
function eXcell_combo(cell){
	...
	this.edit = function(){
		...
		var val_img=this.getValueAndImg();
		var indx=val_img.indexOf("<img");
		this.imgv=indx==-1?"":val_img.substring(indx);
	...
	*/
	...
	this.detach = function(){
		...
		if (!this.combo.getComboText() || this.combo.getComboText().toString()._dhx_trim()==""){
			this.setComboCValue("&nbsp;"+this.imgv);
			this.cell._clearCell=this.imgv=="";
    	}
		else{
			this.setComboCValue(this.combo.getComboText()+this.imgv,this.combo.getActualValue());
