function dhtmlXForm(parentObj, data) {
	/* 解析并获取每个控件对象，主要获取控件的ftype,vtype,name属性
	 * data中的value仅为控件初始化时的value值，控件的value值还是getItemValue方法获取
	 * */
	...
	function getObjList (Obj) {
		if (objExsit(Obj.list)) {
			for (var m = 0; m < Obj.list.length; m++) {
				if ("" != Obj.list[m].vtype && undefined != Obj.list[m].vtype && null != Obj.list[m].vtype) {
					objList[objList.length] = Obj.list[m];
				}
				getObjList(Obj.list[m]);
			}
 		}
	}
	this.i = {
	...
	*********************************************************************************************************************************************/
	...
	this.validate = function () {
		...
		for (var i = 0; i < objList.length; i++) {
			var obj = objList[i];
			var id = objList[i].uid;
			var label = obj.label;
			var content = this.getForm().getItemValue(obj.name);
			if (null == content || undefined == content || "" == content)
				continue;
			switch (obj.vtype) {
			//email类型，邮箱格式必须为xxxxx@yyy.com
			case 'email':
				if (!checkEmail(content, id, label))
					validate_result = false;
				break;
			//电话号码类型，（联系）电话、宅电、传真必须为数字，可以用“—”隔开
			case 'phone':
				if (!checkPhone(content, id, label))
					validate_result = false;
				break;
			//校验是否全由数字组成，例如银行账号、支付宝账号等
			case 'digital':
				if (!checkDigital(content, id, label))
					validate_result = false;
				break;
			//校验邮政编码
			case 'postCode':
				if (!checkMailCode(content, id, label))
					validate_result = false;
				break;
			//校验浮点数类型（包括整数和小数），例如总金额、优惠金额等
			case 'float':
				if (!checkFloat(content, id, label))
					validate_result = false;
				break;
			case 'startDate':
				var j;
				for (j = 0; j < objList.length; j++) {
					if ('endDate' == objList[j].vtype)
						break;
				}
				var start_id = obj.uid;
				var end_id = objList[j].uid;
				var startDate = this.getForm().getCalendar(obj.name).getDate();
				var endDate = this.getForm().getCalendar(objList[j].name).getDate();
				var str_startDate = this.getForm().getInput(obj.name).value;
				var str_endDate = this.getForm().getInput(objList[j].name).value;
				if(typeof str_startDate == "string" && str_startDate == ""){      // 起始时间为空，那么取一个过去最小时间
					startDate = new Date(1970,0,1);
				}
				if(typeof str_endDate == "string" && str_endDate == ""){      // 结束时间为空，那么现在
					endDate = new Date();
				}
				if (!checkDate(startDate, endDate, str_startDate, str_endDate, start_id, end_id))
					validate_result = false;
				break;
			default:
				break;
...
dhtmlxEvent(window, "load", function(){
	var a = [
		"ftype", "vtype", "name", "value", "label", "check", "checked", "disabled", "text", "rows", "select", "selected", "command", "width", "style",
		"labelWidth", "labelHeight", "labelLeft", "labelTop", "inputWidth", "inputHeight", "inputLeft", "inputTop", "position", "size","btnclass","format","dateFormat"
	];
...
function checkTip (patrn, checkCon, id, label) {
	//判断是否该控件存在
	if(id == null) {
		return false;
	}
	//判断是否符合正则表达式
	if (! patrn.test(checkCon)) {
...
function sAlert (focusConId) {
	if (document.getElementById(focusConId) != null) {
		document.getElementById(focusConId).focus();
		document.getElementById(focusConId).style.border = '1px solid red';
	}
}

//1、（联系）电话、宅电、传真必须为数字，可以用“—”隔开
function checkPhone(checkCon, id, label) {
//校验普通电话、传真号码：可以“+”开头，除数字外，可含有“-” 
	var patrn = /^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})+$/;
	return checkTip(patrn, checkCon, id, label + "格式不对");
}

//3、邮箱格式必须为xxxxx@yyy.com
function checkEmail(checkCon, id, label) {
	var patrn=/^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/;
	return checkTip(patrn, checkCon, id, label);
}

function checkDigital (checkCon, id, label) {
	//校验是否全由数字组成 
	var patrn=/^[0-9]{1,20}$/;
	return checkTip(patrn, checkCon, id, label);
}

function checkMailCode (checkCon, id, label) {
	//校验邮政编码 
	var patrn=/^[a-zA-Z0-9]{3,12}$/;
	if (checkCon.length != 6) {
		$func.alert('邮政编码格式不对');
		sAlert(id);
		return false;
	}
...
function checkFloat (checkCon, id, label) {
	var patrn1=/^[0-9]{1,20}$/,
		patrn2=/^(-?\d+)(\.\d+)?$/;
	if (true == patrn1.test(checkCon) || true == patrn2.test(checkCon)) {
		document.getElementById(id).style.border = '';
		return true;
	}
	else {
		if (label[label.length - 1] == ':' || label[label.length - 1] == '：') label = label.substr(0, label.length - 1);
		$func.alert(label + '格式错误！');
		sAlert(id);
		return false;
	}
}

function checkDate(startDate, endDate, str_startDate, str_endDate, start_id, end_id) {
	var datePattern = /^(\d{4})-(\d{1,2})-(\d{1,2})$/;
	if (str_startDate != "" && !checkTip(datePattern, str_startDate, start_id, "开始时间格式错误！")) {
		return false;
	}
  	if (str_endDate != "" && !checkTip(datePattern, str_endDate, end_id, "结束时间格式错误！")) {
  		return false;
	}
  	if (startDate > endDate) {
  		$func.alert("结束时间不能早于开始时间！");
  		return false;
  	}
  	return true;
}

function objExsit (obj) {
