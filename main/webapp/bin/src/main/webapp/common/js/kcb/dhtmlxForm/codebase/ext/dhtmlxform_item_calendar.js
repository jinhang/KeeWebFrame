dhtmlXForm.prototype.items.calendar = {
	...
	render: function(item, data) {
		...
		this.f[item._idd] = (data.dateFormat||"%Y-%m-%d");
		...
		//如果格式包含小时 自动打开time选择器 孙东辉
		if(this.f[item._idd].indexOf("%H")>=0){
			this.calendar[item._idd].showTime();
		}
	...
	getValue: function(item, asString) {
		//当用户手动修改时间时需要重新获取一遍--孙东辉
		var _str = $(item).find("input").val();
		
		if($.trim(_str)!=""){
			var date = this.strToDate(item, _str);
			if(date.getTime()){
				return date;
			}
		}else{
