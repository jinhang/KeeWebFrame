...
dhtmlXGridObject.prototype._page_skin_update = function(name){
	if (!this.pagesInGroup) this.pagesInGroup=Math.ceil(Math.min(5,this.total/this.rowsBufferOutSize));
	var totalPages=Math.ceil(this.total/this.rowsBufferOutSize);
...
dhtmlXGridObject.prototype._pgn_block=function(sep){ 
	...
	var max=Math.min(Math.ceil(this.total/this.rowsBufferOutSize),start+this.pagesInGroup);
	var str=[];
...
dhtmlXGridObject.prototype._pgn_link=function(mode,ac,ds){
	...
	if (mode=="nextpages" || mode=="next"){
		if (this.total/this.rowsBufferOutSize <= this.currentPage ) return ds;
		if (this.total/(this.rowsBufferOutSize*(mode=="next"?'1':this.pagesInGroup)) <= 1 ) return ds;
	...
	if (mode=="current"){
		...
		if (i<1 || Math.ceil(this.total/this.rowsBufferOutSize) < i ) return ds;
...
dhtmlXGridObject.prototype._pgn_bricks = function(page, start, end){
		...
		if(this.total>0 && this.showRecInfo)
			this.pagerElAr["pagerInfoBox"].innerHTML = this.i18n.paging.records+(start+1)+this.i18n.paging.to+end;
