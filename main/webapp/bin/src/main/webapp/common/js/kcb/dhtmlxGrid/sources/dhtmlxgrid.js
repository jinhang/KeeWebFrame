...
//	}; //avoid text select
	...
	this.columnWidthId="";
	this.isCheckHdr=false;
	this.lastMoveHdrCol=-1;
	this.isSortIcon=false;
	this.hdrOptions=[];
	this.columnSortDb=dhtmlxArray();
	this.sortCall=null;
...
//#}
	...
	*/
	this.init=function(fl){
		this.enableRowsHover(true,"rowhover"); 
		...
		if (selMethod == 1){
			...
			if (fl&& typeof (rid) != "undefined" && cid && !skipRowSelection) {
				self.onRowSelectTime=setTimeout(function(){
					if (self.callEvent)
						checkedSelectedRow(self,rid,cid._cellIndex);
...
//#}
		...
		if (this.editor != null){
			...
			if (this.callEvent("onEditCell", [
				...
				//_isIE&&
				if((this.cell.firstChild.type=="textarea"||this.cell.firstChild.type=="text")){
	...
	*/
	this._onHeaderClick=function(e, el){
		var that = this.grid;
		that.isCheckHdr=true;
		if(that.isSortIcon==true&&that.hdr.rows[1]){
			for(var i=0;i<that._cCount;i++)
				that.hdr.rows[1].children[i].className="";
		}
		el=el||that.getFirstParentOfType(_isIE ? event.srcElement : e.target, "TD");
		var option1=eval(that.hdrOptions[el._cellIndex]);
		if(that.cellType[el._cellIndex]=='combo'&&option1&&option1.length>0)return false;
		if(that.isSortIcon==true)that.sortImg.style.display="inline";	
		...
		that.lastMoveHdrCol=el._cellIndex;
	}
...
//#}
	...
	*/
	...
	this.setEditIcon=function(fl){
		this.isEditIcon=convertStringToBoolean(fl);
	}
	
	this.setSortIcon=function(fl){
		this.isSortIcon=convertStringToBoolean(fl);
	}
	
	this.setColumnWidthId=function(id){
		this.columnWidthId=id;
	}
	
	this.setColumnSortDB=function(sortDBStr){
		this.columnSortDb=dhtmlxArray(sortDBStr.split(this.delim));	
	}
	
	this.setSortFunc=function(call){
	...
	*/
	this.setColumnLabel=function(c, label, ind, hdr){
		...
		if (!this.useImagesInHeader){
			var options1=eval(this.hdrOptions[col]);
			if(this.cellType[col]=='combo'&&options1&&options1.length>0){	
				var hdrHTML = "<div class='hdrcell'>";
				hdrHTML += "<select name='hdr_"+ this.columnIds[col] +"' style='width:"+ parseInt(this.initCellWidth[col]-15) +"px'>";
				hdrHTML += "<option value=''>"+ label +"</option>";
				for(var i=0;i<options1.length;i++)
					hdrHTML += "<option value='"+ options1[i] +"'>"+ options1[i] +"</option>";
				hdrHTML+="</select></div>";
	...
	*/
	this.sortField=function(ind, repeatFl, r_el){
		...
		if (el.tagName == "TH"&&(this.fldSort.length-1) >= el._cellIndex
			...
			if(this.columnSortDb[el._cellIndex]==1&&this.sortCall!=null&&typeof(this.sortCall) == "function"){
				this.sortCall(this.columnIds[el._cellIndex],sortType=="des"?"desc":"asc");
			}else{
...
*/
	...
	/*
	 * add by 2013/12/28 10:17:52
	 * Dingmingjin 
	 */
	this.setColEditIcon=function(iconStr){
		this.cellEditIcon=dhtmlxArray(iconStr.split(this.delim));	
	}
	
	this.setHdrOptions=function(optionStr){
		this.hdrOptions=optionStr.split('|');
	}
...
//#column_resize:06042008{
	if ((!_isOpera)||(_OperaRv > 8.5)){
		this.hdr.onmousemove=function(e){
			...
			if(this.grid.isCheckHdr==true)return false;
			if(c._cellIndex==this.grid.lastMoveHdrCol)this.grid.sortImg.style.display="none";	
			...
			if(this.grid.isSortIcon==false)return true;
			for(var i=0;i<this.grid._cCount;i++){
				r.children[i].className=((i==c._cellIndex)?"hoverState":"");
			}
		};
...
//#}		
	...
	*/	
	...
	_insertRowAt:function(r, ind, skip){
		...
		//修改编辑单元格的样式--孙东辉,
		this._showEditCellStyle(r);
		this.rowsCol._dhx_insertAt(ind, r);
		this.callEvent("onRowInserted",[r, ind]);
		return r;
	},
	_showEditCellStyle : function(r){
		for(var i = 0;i<this.cellType.length;i++){
			if(this.cellType[i]=="ed"||this.cellType[i]=="combo"||this.cellType[i]=="dhxCalendar"||this.cellType[i]=="dhxCalendarA"){
				if(this.editableColumn&&$data.arrHasProp(this.editableColumn,this.columnIds[i])){
					continue;
				}
				var className="edit";
				if(this.isEditIcon==true&&this.cellEditIcon[i]==1&&this.cellType[i]=="combo"){
					var exists_img=false;
					for(var j=0;j<r.children[i].childNodes.length;j++){
						if(r.children[i].children[j]&&r.children[i].children[j].tagName=='IMG'&&(r.children[i].children[j].src.indexOf("dyn_.png")!=-1||
								r.children[i].children[j].src.indexOf("sort_desc.gif")!=-1)){
							exists_img=true;
							break;
						}
					}
					if(!exists_img){
						...
						img.src=this.imgURL+"sort_desc.gif";
						r.children[i].appendChild(img);
					}
				}else if(this.isEditIcon==true&&this.cellEditIcon[i]==1){
					className="editEd";
				}
				r.children[i].className = className;
			}
		}
	},
	setEditableColumn : function(c){
		this.editableColumn = c;
	},
	addEditableColumn : function(c){
		if(!this.editableColumn){
			this.editableColumn = [];
		}
		
		var isarrhas = false;
		for(var i = 0;i<this.editableColumn.length;i++){
			if(this.editableColumn[i]==c){
				isarrhas = i;
				break;
			}
		...
		if(!isarrhas){
			this.editableColumn.push(c);
		}
		
		var idindex = -1;
		for(var i = 0;i<this.columnIds.length;i++){
			if(this.columnIds[i]==c){
				idindex = i;
				break;
			}
		}
		...
		for (var i = 0; i < this.rowsBuffer.length; i++){
			this.rowsBuffer[i].children[idindex].className = "";
		}
		
	},
	removeEditableColumn : function(c){
		if(!this.editableColumn){
			this.editableColumn = [];
		}
		...
		if(isarrhas){
			this.editableColumn.splice(isarrhas, 1);
		}
		...
		for (var i = 0; i < this.rowsBuffer.length; i++){
			this.rowsBuffer[i].children[idindex].className = "edit";
		}
	},
...
var checkedSelectedRow = function(mygrid, id, ind) {
	if(!mygrid.getColIndexById("select")){
		return;
	}
	...
	for ( var i = 0; i < allidsarr.length; i++) {
		var c = 0;
		if (arrHasProp(idsarr, allidsarr[i])) {
...
var arrHasProp = function(arr, prop) {
	var falg = false;
