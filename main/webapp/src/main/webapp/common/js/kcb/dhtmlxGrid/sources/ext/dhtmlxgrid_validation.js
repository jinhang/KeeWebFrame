...
dhtmlxValidation.prototype={
	...
	isValidInteger2: function(value) {
		return value==''||!!value.toString().match(/(^-?\d+$)/);
	},
	isValidNumericOrEmpty:function(value) {
		return this.isNotEmpty(value)&&this.isValidNumeric(value)&&value>0;
	},
	isValidNumericOrEmpty2:function(value) {
		return this.isNotEmpty(value)&&this.isValidNumeric(value)&&value>0&&/^(([1-9][0-9]{0,2})|([0-9]{1,3}\.[0-9]{1}))$/.test(value);
	},
	isValidNumeric: function(value) {
		return value==""||!!value.toString().match(/(^-?\d\d*[\.|,]\d*$)|(^-?\d\d*$)|(^-?[\.|,]\d\d*$)/);
	},
