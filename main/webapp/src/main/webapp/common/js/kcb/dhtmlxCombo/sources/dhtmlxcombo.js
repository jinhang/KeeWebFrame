...
function dhtmlXCombo(parent,name,width,optionType,tabIndex){
	...
	dhtmlXCombo.prototype.setComFilter = function(mode){
		
		this._comFilter = true;
	}
	
	dhtmlXCombo.prototype.setDefaultTaga = function(text,color,call){
		var taga=document.createElement("a");
		taga.innerHTML="<u>"+text+"</u>";
		taga.href="javascript:"+call+"();";
		taga.style.color=color;
		taga.style.cssFloat="right";
		taga.style.paddingRight="5px";
		this.default_taga = taga;
	}
	
	dhtmlXCombo.prototype.setCheckedTaga = function(text,color,call){
		var taga=document.createElement("a");
		taga.innerHTML=text;
		taga.href="javascript:"+call+"();";
		taga.style.color=color;
      ...
      this.render(false);
        for (var i=0; i<args.length; i++) {
         ...
         if (attr.length){
               ...
               attr.text = attr[1]||"";
               attr.css = attr[2]||"";
               attr.taga = this.default_taga?(this.default_taga.outerHTML||""):"";
         }
      ...
      dhtmlXCombo.prototype._createSelf = function(selParent, name, width, tab)
      {
         if (width.toString().indexOf("%")!=-1){ 
            ...
            window.setInterval(function(){ 
               ...
               self.setSize(self._lastTs=ts);},10);
...
*/
      dhtmlXCombo.prototype._onKeyF = function(e){
         ...
         if (ev.keyCode=="13" || ev.keyCode=="9" ){
        	 var data=that._selOption.data();
             var text=that.getComboText();
             if (text!=data[1]){
          	   that.setComboText(data[1]);
                
                var len = text.length+1;
                if(that._comFilter){
             	   len = 0;
                }
                dhtmlXRange(that.DOMelem_input,len,data[1].length);
             }
...
*/
      dhtmlXCombo.prototype._onKey = function(e){
		...
		if ((ev!="13")&&(ev!="9")){
         	window.setTimeout(function(){ that._onKeyB(ev); },10);
         	if (ev=="40" || ev=="38")
         		return false;
     	}
         else if (ev==9){
...
*/
      dhtmlXCombo.prototype._fillFromXML = function(obj,b,c,d,xml){
         ...
         for (var i=0; i<options.length; i++) {
            ...
            attr.taga = obj.default_taga?(obj.default_taga.outerHTML||""):"";
      ...
      dhtmlXCombo.prototype.changeCheckTaga = function(){
    	  var val=this.getSelectedValue();
    	  if(val==''||val==null||val==undefined||!this.default_taga||!this.checked_taga)return;
    	  for(var i=0;i<this.optionsArr.length;i++){
    		  var oldText=this.optionsArr[i].text_old;
    		  if(!this.optionsArr[i].value||this.optionsArr[i].value==''||oldText=='全部')continue;
    		  if(this.optionsArr[i].text&&this.getSelectedText()==this.optionsArr[i].text){
    			  this.optionsArr[i].text=oldText+this.checked_taga.outerHTML;
    			  this.optionsArr[i].content.innerHTML=this.optionsArr[i].text;
    		  }else{
    			  this.optionsArr[i].text=oldText+this.default_taga.outerHTML;
    			  this.optionsArr[i].content.innerHTML=this.optionsArr[i].text;
    		  }
    	  }
    	  this.redrawOptions();
...
*/
      dhtmlXCombo.prototype.selectOption = function(ind,filter,conf){
         ...
         if (filter){
          ...
          //     dhtmlXRange(this.DOMelem_input,len,data[1].length);
            }
         }
         else
        	 this.setComboText(data[1]);
...
*/
   dhtmlXCombo.prototype.openSelect = function(){ 
      ...
      //解决在弹出层跑到浏览器下面溢出
     if(window["helps"]){
    	 var kcb_top = parseInt(this.DOMlist.style.top.replace('px',''),10);
     	  if(kcb_top+100>helps.getWindowHeight()){
     		this.DOMlist.style.top = (kcb_top-120)+'px';
     	  }
     }
...
*/
    dhtmlXCombo.prototype.filterSelf = function(mode)
   {
      ...
      var filter=new RegExp(fixx+text.replace(/([\[\]\{\}\(\)\+\*\\\?\.])/g,"\\$1"),"i");
...
dhtmlXCombo_defaultOption.prototype.setValue = function(attr){
    ...
    this.text_old = attr.text||"";
    this.text = attr.text||"";
    this.css = attr.css||"";
    if(this.value!=""){
    	this.text+=(attr.taga||"");
    	this.css+=attr.taga?"padding:6px 2px":"";
    }
    this.content=null;
}
...
*/
      dhtmlXCombo_defaultOption.prototype.data = function(){
         ...
         if (this.content)
            //return [this.value,this._ctext ? this._ctext : this.text_old];
        	 return [this.value , this.text_old];
      }
