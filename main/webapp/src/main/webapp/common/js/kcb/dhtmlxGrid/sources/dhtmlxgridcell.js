...
dhtmlXGridCellObject.prototype.setCValue=function(val, val2){
	if(typeof(val) == 'string'&&val.indexOf("|r|")!=-1&&val.split("|r|").length>1){
		var vals=val.split("|r|");
		val="";
		for(var i=0;i<vals.length;i++)
			val+="<div style=\"padding:10px 0px;\">"+vals[i]+"</div>";
	}
...
function eXcell_ch(cell){
	...
	this.changeState=function(fromClick){
		...
		if (this.grid.callEvent("onEditCell", [
			...
			var selectRows = getCheckedRowsByGrid(this.grid, this.grid.getColIndexById('select'));
			for(var i=0;i<this.grid.hdr.rows[1].cells.length;i++){
				var html_v=this.grid.hdr.rows[1].cells[i].innerHTML;
				if(html_v.indexOf('checkbox')>0){
					var cell_checkBox = this.grid.hdr.rows[1].cells[i].getElementsByTagName("input")[0];
					cell_checkBox.checked=selectRows.length==this.grid.rowsCol.length;
					break;
				}
			}
